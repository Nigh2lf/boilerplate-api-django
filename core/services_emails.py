...
def open_and_return(my_file):
    with open(settings.BASE_DIR_OS + '/core/template_emails/' + my_file, 'r', encoding="utf-8") as file:
        data = file.read()
    return data
...
def send_email_forgot_password(email, name, link):
    ...
    send_smtp_email = sib_api_v3_sdk.SendSmtpEmail(to=email, html_content=html_content, sender=sender, subject=subject)
